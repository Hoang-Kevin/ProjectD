...
updateTask = async (req, res) => {
    ...
    Task.findOne({ _id: req.params.id }, (err, task) => {
        ...
        task.currentProcess = body.currentProcess
        task.ressources = body.ressources
        task
            .save()
            .then(() => {
...
getTaskByProcess = async (req, res) => {
    await Task.find({ currentProcess: req.params.id }, (err, task) => {
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
